...
use worker_pipeline::{run_v3_durable_worker_impl, run_v3_single_writer_impl};
use worker_runtime::{run_durable_notifier, run_v3_loss_monitor};
use workers::{
    affinity_cpu_for_worker, log_v3_worker_topology, parse_cpu_affinity_list_env,
    spawn_v3_background_workers, spawn_v3_tcp_ingress,
};
...
pub async fn run(
    ...
    log_v3_worker_topology(
        &state,
        &v3_shard_affinity_cpus,
        &v3_durable_affinity_cpus,
        &v3_tcp_server_affinity_cpus,
        v3_dedicated_worker_runtime,
    );
    spawn_v3_background_workers(
        &state,
        v3_rxs,
        v3_durable_rxs,
        durable_rx,
        ...
        &v3_durable_affinity_cpus,
        v3_dedicated_worker_runtime,
        v3_durable_worker_batch_adaptive_cfg,
        v3_durable_worker_pressure_cfg,
    );
    ...
    let app = build_http_router(state, v3_http_ingress_enable, v3_http_confirm_enable);
...
async fn run_v3_single_writer(shard_id: usize, rx: Receiver<V3OrderTask>, state: AppState) {
    run_v3_single_writer_impl(shard_id, rx, state).await;
}
...
mod tests {
    ...
    fn v3_confirm_store_rebuilds_from_wal_reader() {
        ...
        let restored = startup_rebuild::rebuild_v3_confirm_store_from_reader(&store, reader, 1024);
    ...
    fn v3_confirm_store_rebuild_keeps_latest_event_without_duplicates() {
        ...
        .join("\n");
        let reader = BufReader::new(Cursor::new(events.into_bytes()));
        let restored = startup_rebuild::rebuild_v3_confirm_store_from_reader(&store, reader, 3);
    ...
    fn collect_v3_rebuild_records_parses_runtime_fields_and_max_seq() {
        ...
        .join("\n");
        let reader = BufReader::new(Cursor::new(events.into_bytes()));
        let (records, max_seq_per_session) =
            startup_rebuild::collect_v3_rebuild_records_from_reader(reader, 1024);
