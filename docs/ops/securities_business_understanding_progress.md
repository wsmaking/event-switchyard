...
さらに、明示的にスコープ外とするものがある。

- 実在の取引所、清算機関、カストディ、銀行、規制当局、情報ベンダーとの接続
- 実在の FIX / native venue session を使った本番相手先通信
- 実在の証券会社、ブローカー、清算機関に対する注文送信や決済連携

ここでの目標は、「実在の外部機関と繋ぐこと」ではない。  
目標は、**外部接続そのものを除けば、現実の証券業務で起きる判断、状態遷移、台帳、運用、障害、説明責務をできるだけ実務に近い形で再現すること**である。
...
つまり、この repo は「現物株の front-to-back を深く理解する」だけでなく、  
市場構造、機関投資家フロー、post-trade、risk、multi-asset、運用工学まで含めた広い実務地図を、  
**外部機関接続を除く repo スコープでは一通り再現できる状態**まで進んだ。  
残るのは、実在の取引所、清算機関、カストディ、銀行、規制当局、情報ベンダーに接続したときにしか発生しない差分と、  
その相手先固有仕様に合わせた接続作業である。
...
このロードマップで言う「実装完了」は、実在の外部機関に接続することを意味しない。  
実在の外部機関接続はスコープ外とし、**それ以外の業務フロー、状態管理、帳票・台帳、リスク、運用監視、障害対応は現実の実務再現を前提に詰める**。
...
- [x] mobile に `執行設計` 画面を追加する
- [x] child order と parent progress を可視化する
...
- [x] asset class ごとの差分を backend catalog に固定し、mobile はその正本を読む
- [x] trade initiation / books and records / failure mode を asset class ごとに分離
...
- backend に asset class catalog を置き、mobile の説明を固定文ではなく正本データへ寄せる
- books and records / settlement / failure mode の差を asset class ごとに保持する

Done 条件:

- equities / options / FX / rates / credit / futures を同じ画面で比較できる
- trade initiation / lifecycle / valuation / settlement / risk driver を asset class ごとに説明できる
- books and records と failure mode の差分を asset class ごとに保持している
- mobile と on-device fallback が同じ asset class catalog を前提に動く

今回ここまで入れたもの:

- [x] `app-java/src/main/java/appjava/asset/AssetClassCatalogService.java`
- [x] `app-java/src/main/java/appjava/mobile/MobileRoadmapService.java`
- [x] `frontend/src/components/mobile/MobileAssetClassView.tsx`
- [x] `frontend/src/offline/mobileRoadmapOffline.ts`
- [x] `frontend/src/types/mobile.ts`

この優先度で増やすファイル:

- `app-java/src/main/java/appjava/asset/AssetClassCatalogService.java`
- `frontend/src/components/mobile/MobileAssetClassView.tsx`
- `frontend/src/offline/mobileRoadmapOffline.ts`
...
- `gateway-rust`
  ...
  - [x] `app-java` 集約を通じて venue session 相当の明示状態を読める
  - [x] `app-java` 集約を通じて throttling / entitlement / rollout state を読める
- `oms-java`
  - [x] stats、bus stats、reconcile、pending orphan、DLQ がある
  - [x] projection lag と schema compatibility の summary を app 層で集約
  - [x] replay / reconcile readiness を rollout gate として app 層に集約
- `backoffice-java`
  - [x] stats、bus stats、reconcile、ledger、pending orphan、DLQ がある
  - [x] settlement / statement / books and records の runtime summary を app 層で集約
  - [x] feed stale 時の risk / valuation guard を books runtime として app 層へ集約
- `app-java`
  - [x] `ProductionEngineeringService` で gateway / projection / feed / schema / capacity を集約
  - [x] `/api/ops/production-engineering` を追加
  - [x] drop copy / venue state / rollout state / throttling state の集約
- `mobile`
  - [x] live runtime と operator sequence を同じ画面に表示
  - [x] session / feed / schema / capacity を live 値で読む導線
  - [x] incident drill を live runtime と結び付けた step-by-step モード
  - [x] throttle / entitlement / rollout を UI で再現
...
- [x] `GET /api/ops/production-engineering`
  ...
  - active incident 一覧
  - operator の確認順
- [x] `GET /api/ops/venue-sessions`
  - venue / drop copy / entitlement / throttling の個別状態
- [x] `GET /api/ops/rollout-state`
...
2. [x] venue / drop copy / entitlement / throttling
3. [x] rollout / schema compatibility / replay readiness gate
4. [x] incident drill と capacity gate の自動化
...
- venue / drop copy / throttling / entitlement / rollout を live runtime と同じ語で読める
- books / settlement / statement / margin guard を運用画面で同時に見られる
...
この文書で定義した優先度 1 から 6 までは、**実在の外部機関接続を除く repo スコープでは実装完了** とみなす。  
ここで言う完了とは、学習用の説明画面だけでなく、backend の正本 read model、API、mobile 表示、on-device fallback、運用手順が揃っていることを指す。

現時点で揃っているもの:

- 市場構造と執行品質
- 機関投資家フローの execution package / allocation / working state
- post-trade の settlement / statement / exception / corporate action workflow
...
- 実在の取引所、清算機関、カストディ、銀行、規制当局、情報ベンダーとの本物接続
- 相手先固有の FIX / native protocol の本番相互接続試験
- 外部機関の認証、審査、契約、証明書発行が必要な運用
