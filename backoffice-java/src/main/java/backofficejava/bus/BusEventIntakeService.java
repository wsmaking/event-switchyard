package backofficejava.bus;

import backofficejava.audit.GatewayAuditEvent;
...
public final class BusEventIntakeService {
    ...
    public BusEventIntakeService(GatewayAuditIntakeService auditIntakeService) {
        ...
        this.enabled = parseBoolean(System.getenv().getOrDefault("BACKOFFICE_BUS_ENABLE", "true"));
        this.kafkaEnabled = parseBoolean(System.getenv().getOrDefault("BACKOFFICE_BUS_KAFKA_ENABLE", "false"));
        this.bootstrapServers = System.getenv().getOrDefault(
            "BACKOFFICE_BUS_KAFKA_BOOTSTRAP_SERVERS",
            System.getenv().getOrDefault("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092")
        );
        this.topic = System.getenv().getOrDefault(
            "BACKOFFICE_BUS_KAFKA_TOPIC",
            System.getenv().getOrDefault("KAFKA_TOPIC", "events")
        );
        this.groupId = System.getenv().getOrDefault("BACKOFFICE_BUS_KAFKA_GROUP_ID", "backoffice-java");
        this.pollMs = Long.parseLong(System.getenv().getOrDefault("BACKOFFICE_BUS_KAFKA_POLL_MS", "500"));
    }
    ...
    public void start() {
        ...
        Thread worker = new Thread(this::runKafkaLoop, "backoffice-java-bus-kafka");
