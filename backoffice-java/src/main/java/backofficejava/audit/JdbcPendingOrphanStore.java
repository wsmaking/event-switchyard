...
public final class JdbcPendingOrphanStore implements PendingOrphanStore {
    ...
    public void append(PendingOrphanEntryView entry) {
        ...
        }, "failed_to_append_backoffice_pending_orphan:" + entry.eventRef());
    }
    ...
    public List<PendingOrphanEntryView> find(String orderId, int limit) {
        ...
        try (
            ...
            throw new IllegalStateException("failed_to_query_backoffice_pending_orphans", exception);
        }
    }

    @Override
    public void removeByEventRef(String eventRef) {
        withTransaction(connection -> deleteByEventRef(connection, eventRef), "failed_to_remove_backoffice_pending_orphan:" + eventRef);
    }

    @Override
    public int size() {
        return count("SELECT COUNT(*) FROM bo_pending_orphan", "failed_to_count_backoffice_pending_orphans");
    }

    @Override
    public void reset() {
        withTransaction(connection -> execute(connection, "DELETE FROM bo_pending_orphan"), "failed_to_reset_backoffice_pending_orphans");
    }
    ...
    private static void deleteByEventRef(Connection connection, String eventRef) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM bo_pending_orphan WHERE event_ref = ?")) {
