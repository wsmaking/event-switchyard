#!/usr/bin/env python3
"""
Open-loop load generator for v3 TCP fixed-frame ingress.

Purpose:
- Keep offered RPS fixed as much as possible (independent from response speed).
- Measure completion/acceptance/latency stats for the TCP hot path.
"""
...
V3_TCP_REQUEST_SIZE = 304
V3_TCP_RESPONSE_SIZE = 32
V3_TCP_TOKEN_MAX_LEN = 256
V3_TCP_TOKEN_OFFSET = 2
V3_TCP_SYMBOL_OFFSET = 258
V3_TCP_SYMBOL_LEN = 16
V3_TCP_SIDE_OFFSET = 274
V3_TCP_TYPE_OFFSET = 275
V3_TCP_QTY_OFFSET = 280
V3_TCP_PRICE_OFFSET = 288
...
def encode_v3_tcp_frame(jwt_token: str) -> bytes:
    ...
    # symbol: "AAPL"
    symbol_bytes = b"AAPL"
    out[V3_TCP_SYMBOL_OFFSET : V3_TCP_SYMBOL_OFFSET + len(symbol_bytes)] = symbol_bytes

    # side=1(BUY), type=1(LIMIT), qty=100, price=15000
    out[V3_TCP_SIDE_OFFSET] = 1
...
def worker_loop(
    ...
    while True:
        if stop_now.is_set():
            break
        try:
            _, account_idx = q.get(timeout=0.05)
        except queue.Empty:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Open-loop v3 TCP load generator")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=29001, help="HTTP port (for reporting only)")
    ...
    print("open_loop_v3_tcp_load")
    print(f"host={args.host}:{args.port}")
    print("path=/v3/tcp")
    print(f"tcp_port={args.tcp_port}")
