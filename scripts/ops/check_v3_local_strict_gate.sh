...
for run in $(seq 1 "$RUNS"); do
  ...
  if [[ -n "$summary_path" ]]; then
    ...
    ack_accepted_p99_ns="$(awk -F= '/^server_hotpath_accepted_p99_ns=/{print $2}' "$summary_path" | tail -n1)"
    if [[ -z "$ack_accepted_p99_ns" ]]; then
      ack_accepted_p99_ns="$(awk -F= '/^server_live_ack_accepted_p99_ns=/{print $2}' "$summary_path" | tail -n1)"
    fi
