...
public final class JdbcDeadLetterStore implements DeadLetterStore {
    ...
    public void append(DeadLetterEntryView entry) {
        withTransaction(connection -> {
            ...
            try (PreparedStatement statement = connection.prepareStatement("""
                INSERT INTO oms_dead_letter (
                    event_ref, aggregate_id, account_id, order_id, event_type, reason, detail, payload, event_at, recorded_at, source
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """)) {
                statement.setString(1, entry.eventRef());
                statement.setString(2, entry.orderId());
                statement.setString(3, entry.accountId());
                statement.setString(4, entry.orderId());
                statement.setString(5, entry.eventType());
                statement.setString(6, entry.reason());
                statement.setString(7, entry.detail());
                statement.setString(8, entry.rawLine());
                statement.setLong(9, entry.eventAt());
                statement.setLong(10, entry.recordedAt());
                statement.setString(11, entry.source());
                statement.executeUpdate();
            }
        }, "failed_to_append_oms_dead_letter:" + entry.eventRef());
    }
    ...
    public List<DeadLetterEntryView> find(String orderId, int limit) {
        ...
        try (
            ...
            throw new IllegalStateException("failed_to_query_oms_dead_letters", exception);
        }
    }

    @Override
    public int size() {
        return count("SELECT COUNT(*) FROM oms_dead_letter", "failed_to_count_oms_dead_letters");
    }
    ...
    public DeadLetterEntryView findByEventRef(String eventRef) {
        ...
        try (
            ...
            throw new IllegalStateException("failed_to_find_oms_dead_letter:" + eventRef, exception);
        }
    }

    @Override
    public void removeByEventRef(String eventRef) {
        withTransaction(connection -> deleteByEventRef(connection, eventRef), "failed_to_remove_oms_dead_letter:" + eventRef);
    }

    @Override
    public void reset() {
        withTransaction(connection -> execute(connection, "DELETE FROM oms_dead_letter"), "failed_to_reset_oms_dead_letters");
    }
    ...
    private static void deleteByEventRef(Connection connection, String eventRef) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM oms_dead_letter WHERE event_ref = ?")) {
            statement.setString(1, eventRef);
            statement.executeUpdate();
        }
    }
