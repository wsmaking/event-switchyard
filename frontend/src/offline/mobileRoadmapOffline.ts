    ...
    assetClasses: [
      {
        ...
        sampleMetrics: [
          { label: 'sample fill', value: '100,000株 @ 2,842.5円', note: 'arrival benchmark と平均約定単価の差を execution quality へ載せる' },
          { label: 'settlement cash', value: '-284,250,000円', note: 'buy side の gross cash out' },
          { label: 'risk lens', value: 'single-name 41.6%', note: 'concentration と liquidity が主因' },
        ],
        lifecycleBreakpoints: ['trade date', 'settlement date', 'record date / ex-date'],
      ...
      },
      {
        ...
        sampleMetrics: [
          { label: 'premium', value: '42.8円 x 10枚', note: 'premium cashflow と underlying を別管理' },
          { label: 'delta-equivalent', value: '325株相当', note: 'hedge intuition は株数換算で語る' },
          { label: 'theta', value: '-0.62 / 日', note: '時間経過で premium が削れる' },
        ],
        lifecycleBreakpoints: ['trade date', 'premium settlement', 'exercise / assignment cutoff'],
      ...
      },
      {
        ...
        sampleMetrics: [
          { label: 'spot trade', value: 'USD/JPY 2M @ 151.20', note: 'base/quote の二本 leg を同時に持つ' },
          { label: 'cash legs', value: '+2,000,000 USD / -302,400,000 JPY', note: '片側だけの ledger に押し込まない' },
          { label: 'cutoff', value: 'Tokyo 15:30', note: 'cutoff 超過は next-day settlement に波及' },
        ],
        lifecycleBreakpoints: ['trade date', 'value date', 'holiday-adjusted settlement'],
      ...
      },
      {
        ...
        sampleMetrics: [
          { label: 'clean price', value: '99.42', note: 'clean / dirty price を分ける' },
          { label: 'accrued', value: '0.31', note: 'coupon accrual は settlement 後追いにしない' },
          { label: 'DV01', value: '18,400 USD/bp', note: 'risk は price ではなく rate sensitivity で見る' },
        ],
        lifecycleBreakpoints: ['trade date', 'coupon accrual', 'settlement convention'],
      ...
      },
      {
        ...
        sampleMetrics: [
          { label: 'spread', value: '235bp', note: 'price より spread と recovery 前提で語る' },
          { label: 'jump-to-default', value: '-12.4M円', note: 'single event で PnL が飛ぶ' },
          { label: 'coupon accrual', value: '0.44', note: 'credit でも accrual は別線で残す' },
        ],
        lifecycleBreakpoints: ['trade date', 'coupon accrual', 'credit event watch'],
      ...
      },
      {
        ...
        sampleMetrics: [
          { label: 'contract', value: 'TOPIX JUN26 x 120枚', note: 'contract calendar を symbol と分けて持つ' },
          { label: 'variation margin', value: '+4,320,000円', note: 'daily settlement が現物と決定的に違う' },
          { label: 'initial margin', value: '18,000,000円', note: 'execution 後に margin workflow が主役になる' },
        ],
        lifecycleBreakpoints: ['trade date', 'daily settlement', 'roll date'],
...
export function getOfflineMobileOperationsGuide(): MobileOperationsGuide {
  ...
  return {
    ...
    goNoGo: {
      state: 'GO',
      blockedReasons: ['大きな blocked reason なし'],
      requiredDrills: ['steady-state observation'],
    },
    rolloutState: {
      state: 'READY',
