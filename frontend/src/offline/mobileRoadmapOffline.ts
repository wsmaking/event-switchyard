...
export function getOfflineMobileOperationsGuide(): MobileOperationsGuide {
  ...
  return {
    ...
    sessionMonitors: [
      { name: 'Gateway / Venue session', state: 'RUNNING', whyItMatters: '受注は続いても venue 状態が悪いと執行説明が崩れる', checkpoints: ['heartbeat', 'cancel ack latency', 'venue state explanation'] },
      { name: 'OMS projection', state: ops.omsStats?.state ?? 'ON_DEVICE', whyItMatters: 'accepted / cancel / fill の順序を守る壁', checkpoints: ['aggregate progress', 'pending orphan', 'DLQ'] },
      { name: 'BackOffice projection', state: ops.backOfficeStats?.state ?? 'ON_DEVICE', whyItMatters: 'ledger truth と cash / position の正本', checkpoints: ['fill count', 'ledger entries', 'reconcile issues'] },
      { name: 'Market data freshness', state: 'SIMULATED', whyItMatters: '価格 stale は execution と risk の両方を薄くする', checkpoints: ['tick freshness', 'venue state', 'price gap reason'] },
    ],
    incidentDrills: [
      { name: 'sequence gap 増加', trigger: 'pending orphan が増え続ける', firstQuestions: ['accepted 前提が欠けているか', 'aggregate progress が止まっていないか'], actions: ['pending と DLQ を分けて数える', 'replay 前に raw event 順序を確認'], recoverySignal: 'pending orphan が解消し aggregate progress が進む' },
      { name: 'DLQ 増加', trigger: 'decode 失敗や schema mismatch', firstQuestions: ['payload が壊れているか', 'schema 想定が古いか'], actions: ['DLQ payload を保存', '互換性確認後に requeue'], recoverySignal: '同じ eventRef が再び DLQ に戻らない' },
      { name: 'market data stale', trigger: '価格は見えるが tick が止まる', firstQuestions: ['risk 数字が古くないか', 'arrival benchmark と current price が混ざっていないか'], actions: ['stale 表示を優先', 'execution / risk を保守モードで読む'], recoverySignal: 'fresh tick と venue state が復帰する' },
      { name: 'schema rollout', trigger: 'bus event 項目追加', firstQuestions: ['consumer 互換が保たれているか', 'default 値が定義されているか'], actions: ['backward compatible を先に配る', 'replay drill を事前実施'], recoverySignal: 'old/new payload の双方で projection が壊れない' },
    ],
    schemaControls: [
      { title: 'Event contract', rule: '新規項目は optional / additive を原則とする', failureMode: 'old consumer が decode 不能になり DLQ が急増する' },
      { title: 'Replay safety', rule: 'schema 変更時は replay script と checkpoint 互換を先に確認する', failureMode: '過去 audit が読めず recovery が失敗する' },
      { title: 'Operator wording', rule: 'UI 表示名を変えても raw event / source field は維持する', failureMode: 'runbook と現場画面の用語が噛み合わなくなる' },
    ],
    capacityControls: [
      { title: 'Order intake latency', metric: 'hot path latency', threshold: '< 1ms class を維持', whyItMatters: '説明系 aggregation を gateway に混ぜない理由' },
      { title: 'Projection backlog', metric: 'pending orphan / DLQ / current offset', threshold: '増加傾向なら projection 側を疑う', whyItMatters: 'offset だけでは安全と言えない' },
      { title: 'Recovery time', metric: 'snapshot restore + replay duration', threshold: '数分以内を目標', whyItMatters: '事故後に再説明できるまでの時間を短くする' },
      { title: 'UI trust signal', metric: 'final-out completeness', threshold: 'fills / reservation / ledger が揃ってから成功判定', whyItMatters: '中途半端な画面で話を閉じない' },
    ],
    implementationAnchors: [
